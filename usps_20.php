...
//            "Express Hold for Pickup Commercial"=>"EXPRESS HFP COMMERCIAL"
			 "Priority Mail Int" => "Priority Mail International",
              //"Priority Mail International" => "PRIORITYINTL", // SERVICE ID 2
              "Express Mail Int" => "Express Mail International", // SERVICE ID 1
        );
...
//        $base["SpecialServices"] = "";  // Its here, not ready for it yet, think ASH 1.0 or higher.
    ...
    function _build_intl_shipment(&$request, $data, $package){
        ...
        $min_width = 14;
        if ($package->width < $min_width) {
          $new_width = 14;
        }else{
          $new_width = $package->width;
        }
        
    	$min_length = 14;
        if ($package->length < $min_length) {
          $new_length = 14;
        }else{
          $new_length = $package->length;
        }        
